...
public class JobCheckNavigationHistory implements IActionConfiguration, IJobConfiguration {
    ...
    public static void start() {
        LoggerFacade.getDefault().info(JobCheckNavigationHistory.class, "Start job for History-Navigation...");
        
        ptChecker = new PauseTransition();
        ptChecker.setDuration(CHECKER_DURATION);
        ptChecker.setOnFinished((ActionEvent event) -> {
            ActionFacade.getDefault().handle(ACTION__JOB_CHECK_NAVIGATION__HISTORY);
    ...
    public static void stop() {
        LoggerFacade.getDefault().info(JobCheckNavigationDreamBook.class, "Stop job for History-Navigation");
